      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        <Flex style={styles.validateFlex}>
          <Block style={styles.heading}>
            <Heading variants="h2">
              Enter A Cash Balance Refund Amount Below.
            </Heading>
          </Block>
          <Input
        ...
        <Flex>
          <Button
            ...
            size="giant"
            onPress={() => {
              WriteLog("RefundCash cashTotal" + cashTotal);
          ...
          >
            Refund Cash Balance From Users Wristband
          </Button>
        </Flex>
        <Flex >
          <Block style={{ top: 5 }}>
...
function RefundCashFromWristband({
  closeModal,
  onSuccess,
  cashTotal,
  formattedCashTotal,
  ...
  const lookupRFID = async (rfidUID) => {
    try {
      ...
      WriteLog("RefundCash error" + error);
      console.log({ error });
      updateScanState({
  ...
  const updateBalance = async () => {
    ...
    const variables = {
      ...
      cashBalance: (formattedCashBalance - formattedCashTotal) > 0 ? (formattedCashBalance - formattedCashTotal) : 0,
      addedCash: (formattedCashBalance - formattedCashTotal) > 0 ? Number(cashTotal) : formattedCashBalance,
    ...
    try {
      ...
      WriteLog("RefundCash update balance error" + error);
  ...
  useEffect(() => {
    const subscription = DeviceEventEmitter.addListener(
      "onRfidScanResult",
      async (event) => {
        WriteLog("RefundCash RFID SCAN RESULT" + event);
        ...
        if (regHex.test(newEvent) && newEvent.length >= 8) {
          ...
          WriteLog("RefundCash RFID RESULT - SET DECLINE");
  ...
  const scanRFID = async () => {
    try {
      ...
      WriteLog("RefundCash scan called error");
  ...
  WriteLog("RefundCash", { success: success, error: error });
      ...
      {confirmAddCash ? (
        ...
        >
          {success ? (
            ...
            >
              <Heading variants="h2">New Cash Balance</Heading>
              <Heading variants="h1" style={{ marginTop: 15 }}>
                {`${formatTotals(currentCashBalance)}`}
              </Heading>
            </Block>
          ) : error ? (
            <Block
              ...
              {updatingAssets ? (
                ...
                <>
                  ...
                  <Heading variants="h1" style={{ marginTop: 10 }}>
                    {`${formatTotals(cashTotal > currentCashBalance ? currentCashBalance : cashTotal)}`}
                  </Heading>
...
function RefundCashContainerWithPassword() {
                ...
                Alert.alert("", `Invalid Pin, Please try again.`, [
                  {
                    text: "Close",
                    onPress: () => {
                      WriteLog("RefundCash  Invalid Pin, Please try again.");
...
export default RefundCashContainerWithPassword;
