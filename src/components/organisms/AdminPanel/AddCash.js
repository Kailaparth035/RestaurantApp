      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        <Flex style={styles.validateFlex}>
          <Block style={styles.heading}>
            <Heading variants="h2">Enter A Cash Balance Amount Below.</Heading>
          </Block>
          <Input
        ...
        <Flex>
          ...
          >
            Add Cash Balance to Users Wristband
          </Button>
        </Flex>
      </ScrollView>
    </Box>
...
function AddCashToWristband({
  ...
  const updateBalance = async () => {
    ...
    const variables = {
      ...
      cashBalance: Number(cashTotal) > 0 ? formattedCashBalance + formattedCashTotal : 0,
      addedCash: Number(cashTotal) > 0 ? formattedCashTotal : 0,
    ...
    try {
      ...
      WriteLog("Addcash update balance error" + error);
  ...
  useEffect(() => {
    const subscription = DeviceEventEmitter.addListener(
      "onRfidScanResult",
      async (event) => {
        WriteLog("Addcash RFID SCAN RESULT" + event);
  ...
  WriteLog("Addcash" + { success: success, error: error });
...
function AddCashContainerWithPassword() {
                ...
                Alert.alert("", `Invalid Pin, Please try again.`, [
                  {
                    text: "Close",
                    onPress: () => {
                      WriteLog("Invalid Pin, Please try again.");
...
export default AddCashContainerWithPassword;
